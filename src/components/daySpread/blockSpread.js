...
const blockTarget = {
	drop(props, monitor, component) {
		component.handleCardDroppedOnBlock(
			props.block.id,
...
class BlockSpread extends Component {
	...
	createCards() {
		...
		if (this.props.block.selectedCard) {
			const selected = this.props.block.cards.find(
		...
		for (let i = 0; i < 3; i++) {
			...
			cards.push(
				<Location
					info={this.props.block.cards[i]}
					key={`${this.props.block.id}_${i}`}
					blockId={this.props.block.id}
					cards={cardIds}
				/>
			);
		...
		if (cards.length === 0) {
			cards = (
				<h4>
					Your Ambassador is putting together personalized suggestions
					for{' '}
