...
export const fetchBlocksError = err => ({ type: FETCH_BLOCKS_ERROR, err });
...
export const sendNewBlockRequest = () => ({ type: SEND_NEW_BLOCK_REQUEST });

// on sucess lets push the new block to the state -m instead of remaking the
// block request -m
...
export const sendNewBlockError = err => ({ type: SEND_NEW_BLOCK_ERROR, err });
...
export const putCardOnBlock = ids => (dispatch, getState) => {
  ...
  return fetch(`${API_BASE_URL}/block/${ids.blockID}/cards`, {
    ...
    body: JSON.stringify({ card: ids.cardID })
  })
    .then(response => response.json())
    .then(updatedBlock => {
...
export const selectCardOnBlock = ids => (dispatch, getState) => {
  ...
  return fetch(`${API_BASE_URL}/block/${ids.blockID}/select`, {
    ...
    body: JSON.stringify({ selection: ids.cardID })
  })
    .then(response => response.json())
    .then(updatedBlock => {
      const formattedBlock = Object.assign({}, updatedBlock, {
...
export const REMOVE_CARD_ON_BLOCK_SUCCESS =
  "REMOVE_SELECT_CARD_ON_BLOCK_SUCCESS";
export const removeCardOnBlockSuccess = blockId => ({
  type: REMOVE_CARD_ON_BLOCK_SUCCESS,
  blockId
});

export const removeCardOnBlock = ({ cardID, blockID, cards }) => (
  dispatch,
  getState
) => {
  const authToken = getState().auth.authToken;
  return fetch(`${API_BASE_URL}/block/${blockID}/removecard`, {
    ...
    body: JSON.stringify({ cards, cardID })
  })
    .then(response => response.json())
    .then(updatedBlock => {
      const formattedBlock = Object.assign({}, updatedBlock, {
