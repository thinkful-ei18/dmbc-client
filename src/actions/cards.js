...
export const fetchCardsError = err => ({ type: FETCH_CARDS_ERROR, err });
...
export const fetchFilterCardsSuccess = cards => ({
  type: FETCH_FILTER_CARDS_SUCCESS,
  cards
});
...
export const fetchSearchCards = values => (dispatch, getState) => {
  ...
  dispatch(fetchCardsRequest());
  return fetch(
    `${API_BASE_URL}/cards?searchTerm=${values.searchTerm}&lat=${
      values.lat
    }&distance=${values.distance}&lng=${values.lng}`,
    {
      method: "GET",
      headers: {
        Authorization: `Bearer ${authToken}`
...
export const fetchDestinationCards = destination => (dispatch, getState) => {
  ...
  dispatch(fetchCardsRequest());
  return fetch(
    `${API_BASE_URL}/cards?lat=${lat}&distance=${distance}&lng=${lng}`,
    {
      method: "GET",
      headers: {
        Authorization: `Bearer ${authToken}`
...
export const updateCard = (newCard, cardID, distance) => (
  dispatch,
  getState
) => {
  const authToken = getState().auth.authToken;
  ...
  return (
    fetch(`${API_BASE_URL}/cards/${cardID}`, {
      ...
      // .then(card => {   dispatch(fetchSingleCardSuccess(card)); })
      .then(res => {
        dispatch(
          fetchDestinationCards({
            distance: distance,
